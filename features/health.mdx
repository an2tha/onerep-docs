...
Custom metrics can read themselves off your phone. When you create one, tap **Fill from Apple Health** and pick a catalogue entry: blood glucose, blood pressure, caffeine, VO2 max, and everything else the two stores expose beyond the ten OneRep scores on.
...
A bound metric is labelled **Read from Apple Health** wherever it appears. See [Progress](/features/progress) for the rest of the metric builder, and [the catalogue](/features/health-metrics) for every signal you can point one at.

<Note>
  Custom metrics are moving onto this page, to be grouped and scored alongside the readings already here rather than sitting apart in Progress. The metrics you have keep their history and their bindings through the move, and the API surface does not change. The layout will be documented once it stops moving.
</Note>
