...
<Tabs>
  ...
  <Tab title="Native Apps">
    The iOS and Android Capacitor projects are already checked in under `apps/mobile/ios` and `apps/mobile/android`, so there is nothing to generate. You need Xcode (for iOS) or Android Studio (for Android), plus Bun installed on your machine.
