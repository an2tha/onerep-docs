...
<Note>
  **Deleting needs a separate scope.** The `DELETE` routes require a **Full access** key, and they remove one row at a time by id. There are no bulk-clear operations. Every delete is undoable from the app.
</Note>
