...
<Steps>
  ...
  <Step title="Choose a scope">
    Select **Read only** or **Read & write** depending on what your integration needs to do. Read only is the safer default, since most integrations only ever read. You cannot change the scope after the key is created, so pick carefully.
  </Step>
