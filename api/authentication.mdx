---
title: "OneRep API Authentication: Keys, Scopes, and Limits"
sidebarTitle: "Authentication"
description: "Create and manage OneRep API keys, choose the right scope for your integration, and authenticate every REST or MCP request with a Bearer token."
---
...
Before you make any API calls, locate your personal deployment URL. Open **Settings → API & MCP** in the OneRep app, where your base URL is displayed. Copy it directly from the app rather than trying to construct it by hand.
...
<Steps>
  ...
  <Step title="Choose a scope">
    Select **Read only**, **Read & write**, or **Full access** depending on what your integration needs to do. The scopes are a ladder rather than a set of switches: write includes read, and full access includes both. Read only is the safer default, since most integrations only ever read. You cannot change the scope after the key is created, so pick carefully.
  </Step>
  <Step title="Copy the key immediately">
    After the key is created it is displayed exactly once. Copy it now and store it somewhere secure, such as a password manager or an environment variable in your deployment. OneRep stores only a hash of the key and cannot show it to you again.
  </Step>
</Steps>
...
- **Stored hashed:** OneRep stores a SHA-256 hash, so nobody can read the original key back out
...
|-------|----------------|
| **Read only** | All `GET` routes: days, workouts, measurements, health readings, custom metrics, the bindable catalogue, goals, insights |
| **Read & write** | All `GET` routes **plus** all `POST` routes: logging food, water, weight, check-ins, workouts, rest days, correcting a day's health readings, and defining or filling custom metrics |
| **Full access** | Everything above **plus** the `DELETE` routes: removing entries, sessions, check-ins, days of health readings, and custom metrics |
