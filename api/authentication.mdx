...
|-------|----------------|
| **Read only** | All `GET` routes: days, workouts, measurements, health readings, goals, insights |
| **Read & write** | All `GET` routes **plus** all `POST` routes: logging food, water, weight, check-ins, workouts, rest days, and correcting a day's health readings |
| **Full access** | Everything above **plus** the `DELETE` routes: removing entries, sessions, check-ins, and days of health readings |
