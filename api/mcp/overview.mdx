...
<Note>
A token is only shown the tools its scopes cover. A read-only token lists 10 tools; read & write lists 21; full access lists all 29. Tools outside your scopes are hidden entirely, and calling one directly is refused. Count the entries in `tools/list` if you are unsure which kind of key you are holding.
</Note>
