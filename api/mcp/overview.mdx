...
<Note>
A read-only token lists only the 6 read tools. The 5 write tools are hidden from it entirely, and calling one directly is refused. If `tools/list` returns only 6 entries, you have a read-only token. Create a read & write token to unlock all 11 tools.
</Note>
