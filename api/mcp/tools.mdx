---
title: "Complete MCP Tool Reference for OneRep: All 23 Tools"
sidebarTitle: "Tools"
description: "Complete reference for all 23 OneRep MCP tools: 8 read tools for querying your log, 8 write tools for adding and correcting entries, and 7 delete tools for removing them."
---

OneRep exposes 23 tools over MCP. They are deliberately coarse, so an assistant can describe your training and nutrition without burning its context window on dozens of thin wrappers. Each tool belongs to exactly one scope, and a token is only shown the tools its scopes cover. A read-only token will not list or accept a write or delete tool call.
...
These eight tools require the `write` scope. They are not listed to, or callable by, a read-only token.
